...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'val' == self.datatype:
            ...
            train_frames, train_anno, meta_obj_train = dataset.get_frames(seq_id, train_frame_ids, seq_info_dict,dtype='train_val')
            test_frames, test_anno, meta_obj_test = dataset.get_frames(seq_id, test_frame_ids, seq_info_dict,dtype='test_val')
            ...
            if is_video_dataset:
                ...
                while not enough_visible_frames:
                    # Sample a sequence
                    seq_id = random.randint(0, dataset.get_num_sequences() - 1)
                    frame_list = dataset.sample_dict[seq_id]
                    if len(frame_list) != 0:
                        train_frame_ids = random.choices(frame_list,k=self.num_train_frames)
                        test_frame_ids = random.choices(frame_list,k=self.num_test_frames)
                ...
                test_frame_ids = [1] * self.num_test_frames
            seq_info_dict = dataset.get_sequence_info(seq_id)
            train_frames, train_anno, meta_obj_train = dataset.get_frames(seq_id, train_frame_ids, seq_info_dict,dtype='train')
            test_frames, test_anno, meta_obj_test = dataset.get_frames(seq_id, test_frame_ids, seq_info_dict,dtype='test')
...
class LWLSampler(torch.utils.data.Dataset):
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None):
        ...
        args:
            visible - 1d Tensor indicating whether target is visible for each frame
            num_ids - number of frames to be samples
            min_id - Minimum alowed frame number
            max_id - Maximum allowed frame number
