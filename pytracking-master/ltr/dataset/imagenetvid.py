...
class ImagenetVID(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=default_image_loader, min_length=0, max_target_area=1):
        ...
        if os.path.isfile(cache_file): #add and False
            # If available, load the pre-process ed cache file containing meta-info for each sequence
    ...
    def _get_sequence_path(self,seq_id):
        ...
        seq_path = os.path.join(self.root, 'Data', 'VID', 'train', set_name, vid_name)
        return seq_path 
    ...
    def _process_anno(self, root):
        ...
        for set in sorted(os.listdir(base_vid_anno_path)):
            ...
            for vid in sorted(os.listdir(os.path.join(base_vid_anno_path, set))):
                ...
                #     print('here') 
                # else:
                #     print(os.path.join(base_vid_anno_path, set, vid))
